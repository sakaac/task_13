...
def restaurant_favorite(request, restaurant_id):
	restaurant = Restaurant.object.get(id=restaurant_id)
	favorite, created = FavoriteRestaurant.object.get_or_create(restaurant=restaurant, user=request.user)
	if created == True:
		action = "favorite"
	else:
		favorite.delete()
...
def restaurant_is_favorite(request):
    if not request.is_restaurant_favorite() or not request.method=='POST':
